...
class EncoderSelfAttn(nn.Module):  # 1d conv on query, 1d conv on key
    ...
    def volume_kernel(self, query, key, value, tokenizer, mask=None):
        ...
        value = value.view(_B, -1, _C)
        # print('ttttttttttttttt', query.shape, value.shape)
        ...
        # print('lllllllllllllllllll', mixed_query_layer.shape, mixed_value_layer.shape)
        ...
        # print('xxxxxxxxxxxxxxx', query_layer.shape, value_layer.shape)
        ...
        # print('yyyyyyyyyyyyyyy', context_layer.shape)
...
def make_model(args_out, DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, locs, nb_head, num_of_weeks,
    ...
    args = get_default_args(args_out, nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout,
        adj_mx, norm_Adj_matrix, locs, num_layers, decoder_output_size)


    encoder_embedding = nn.Sequential(nn.Linear(encoder_input_size, d_model),
        TemporalPositionalEncoding(d_model, dropout, max_len, en_lookup_index),
        SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN_PE(norm_Adj_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
    )
    decoder_embedding = nn.Sequential(nn.Linear(decoder_output_size, d_model),
        TemporalPositionalEncoding(d_model, dropout, num_for_predict),
        SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN_PE(norm_Adj_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
    )
...
def get_default_args(args_out, nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout, adj_mx, norm_Adj_matrix, locs, num_layers, decoder_output_size):

    args = D()
    args.args_out = args_out
    args.adjs = build_non_iso_adjs(args_out, adj_mx, locs)
    ...
    elif dataset=='BERLIN_DT':  # go to [def tokenizer] to find answer
        # args._x2 = 2
        # args.dim_token                  =           324
        # args.dim_hidden_A               =           args.dim_token * args._x2
        args.dim_hidden_A               =           768
