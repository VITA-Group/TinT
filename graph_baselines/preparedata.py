...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    train_x = np.concatenate(training_set[:-2], axis=-1).astype(np.float32)  # (B,N,F,T')
    val_x = np.concatenate(validation_set[:-2], axis=-1).astype(np.float32)
    test_x = np.concatenate(testing_set[:-2], axis=-1).astype(np.float32)
    ...
    stats = normalization(train_x, val_x, test_x)

    all_data = {
        'train': {
            'x': (train_x-stats['_mean'])/stats['_std'],
            'target': train_target,
            'timestamp': train_timestamp,
        },
        'val': {
            'x':  (val_x-stats['_mean'])/stats['_std'],
            'target': val_target,
            'timestamp': val_timestamp,
        },
        'test': {
            'x':  (test_x-stats['_mean'])/stats['_std'],
            'target': test_target,
